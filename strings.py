###TAKEN and modified FROM https://github.com/danielvarga/string-art 
...
def build_arc_adjecency_matrix(n, radius):
    ...
    #hooks = np.concatenate( [ np.array([[math.cos(np.pi*2*i/n-tanangle), math.sin(np.pi*2*i/n-tanangle)] for i in range(n)]),
    #                    np.array(
    #                        [[math.cos(np.pi * 2 * i / n + tanangle), math.sin(np.pi * 2 * i / n + tanangle)] for i in
    #                         range(n)]) ] ,axis=0 )
    #hooks =  np.array([[math.cos(np.pi * 2 * i / n - tanangle), math.sin(np.pi * 2 * i / n - tanangle)] for i in range(n)])
    hooks = []
    for i in range(n):
        hooks.append( [math.cos(np.pi * 2 * (i+0.5) / n - tanangle), math.sin(np.pi * 2 * (i+0.5) / n - tanangle)]  )
        hooks.append([math.cos(np.pi * 2 * (i+0.5) / n + tanangle), math.sin(np.pi * 2 * (i+0.5) / n + tanangle)])

    hooks = np.stack( hooks )
